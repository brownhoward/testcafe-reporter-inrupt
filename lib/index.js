...
module.exports = function () {
  ...
  return {
    ...
    reportTaskStart(startTime, userAgents, testCount) {
      ...
      this.includeFooter = "true" == envs("TESTCAFE_REPORT_INCLUDEFOOTER", "true"); // Slack

      this.toSlack = "true" == envs("TESTCAFE_REPORT_TOSLACK", "false"); // Create the new Report

      this.report = new report(this.mergeReport, this.toConsole, this.toSlack);
      this.startTime = startTime;
      this.testCount = testCount; // Output header
      ...
      if (this.includeHeader) {
        ...
        console.log(""); // Output the Report header
    ...
    reportTestDone(name, testRunInfo, meta) {
      // Test Duration
      const durationStr = this.fmtTime(testRunInfo.durationMs); // Errors? Warnings?

      let resultIcon = "";
      if (testRunInfo.errs.length > 0) resultIcon = ":red_circle:";else if (testRunInfo.skippedCount > 0) resultIcon = ":white_circle:";else if (testRunInfo.warnings !== null && testRunInfo.warnings !== undefined && testRunInfo.warnings.length > 0 || testRunInfo.unstable) resultIcon = ":large_yellow_circle:";else resultIcon = ":large_green_circle:"; // Test Result

      let message = resultIcon + " " + name + " (" + durationStr + ")"; // Unstable?

      if (testRunInfo.unstable) message += " (Unstable)"; // Append to Report

      this.report.addMessage(message); // Include Error details?
